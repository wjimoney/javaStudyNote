...
```sql
update tb_user set last_name=?, age=?, where id = 2342
```
...
FROM `tb_clue` c
LEFT JOIN `tb_assign_record` ar ON c.id = ar.assign_id AND ar.type = 0 AND ar.latest = 1
```
 ...
 * 修改商机
 */
@Log(title = "商机", businessType = BusinessType.UPDATE) //切入点
@PutMapping
...
```asciiarmor
注意类中属性的顺序必须和excel中的列保持一致，这样最简单
```
...
仿照官网的demo构建一个监听类ExcelListener,并且重写里面的两个方法
...
我们的需求是什么很简单，将Excel中所有的数据都要存入数据库中

那么如何实现呢？

> 思路：
>
> ​	1）我们应该在这个监听器里调用service或mapper对象来执行Insert操作
>
> ​	2）将成功条数或失败条数返回给前端
...
public class ExcelListener extends AnalysisEventListener<TbClueExcelVo> {
  /**
	...
	@Override
	public void doAfterAllAnalysed(AnalysisContext context) {
		//TODO 由于在单条执行的事件里已经进行了数据的入库
		//TODO 在所有数据都插入完成后就不用再进行插入了
	}
}
```

> 第一个难点，我们要在监听器里进行数据库的插入操作：
>
> 那么我们在监听器里就应该拿到对应的service和mapper对象，如何才能拿到呢？

```asciiarmor
同学们可能会想利用@Autowired去注入，但是如果要注入一个service或mapper：那么首先我们这个监听器就应该被添加到IOC容器当中，否则无法进行@Autowired
```

> 思路：通过contntroller去拿，利用构造方法传入到每次new的监听器里
>

controller能进行@Autowired，那么在controller注入一个我们需要的service

对于监听器里我们还需要创建一个代参构造器，在带参构造器里，将service指向成员属性的service
...
ExcelListener：**新增构造方法，接收service对象**
...
修改代码：TbController，将tbClueService对象通过构造方法传递到listener中
...
需要构建一个实体类ImportResultDTO用来接收本次成功多少条，失败多少条

```
com.huike.clues.domain.dto.ImportResultDTO
```

在TbClueServiceImpl.importCluesData()中实现线索保存：
...
public ImportResultDTO importCluesData(TbClueExcelVo data) {
    ...
    if (StringUtils.isNoneBlank(activityCode)) {
        //TODO 可以优化的点--学生最好自己能优化--利用空间换时间
        //提示：项目启动时将所有的活动id和编码存储到Redis，每次判断直接到内存Redis中，效果更高
    ...
    try {
       //2 判断手机号是否为空
        ...
        //4 验证是否存在这个用户：通过手机号而不是姓名
        TbClue dbcule = tbClueMapper.selectTbClueByPhone(clue.getPhone());
        if (dbcule == null) {
            //5 特殊字段处理
            if (StringUtils.isNoneBlank(clue.getChannel())) {
                //select dict_value from sys_dict_data where dict_type =
                // 'clues_item' and dict_label = '线上活动'
   String channel = sysDictDataMapper.selectDictValue(
         TbClue.ImportDictType.CHANNEL.getDictType(), //'clues_item'
         clue.getChannel()); //将Excel中渠道来源-》转换成-》字典表中的value
                //从Redis中获取字典表中常用的数据
                //String dictValue =
                // DictUtils.getDictValue(TbClue.ImportDictType.CHANNEL.getDictType(),
                //        clue.getChannel());
                clue.setChannel(channel);
}

            if (StringUtils.isNoneBlank(clue.getSubject())) {
                String subject = sysDictDataMapper.selectDictValue(
                      TbClue.ImportDictType.SUBJECT.getDictType(),
                        clue.getSubject()); //将Excel中意向学科-》转换成-》字典表中的value
                clue.setSubject(subject);
            }

            if (StringUtils.isNoneBlank(clue.getLevel())) {
                String level = sysDictDataMapper.selectDictValue(
                      TbClue.ImportDictType.LEVEL.getDictType(),
                        clue.getLevel()); //将Excel中意向级别-》转换成-》字典表中的value
                clue.setLevel(level);
            }

            if (StringUtils.isNoneBlank(clue.getSex())) {
                //0：false 女， 1： true 男
                String sex = sysDictDataMapper.selectDictValue(
                      TbClue.ImportDictType.SEX.getDictType(),
                        clue.getSex()); //将Excel中性别-》转换成-》字典表中的value
                clue.setSex(sex);
            }
...
> 新的问题：我们的事件是单条插入，每次只能知道这条数据有没有成功，如果说是最后所有数据一次性导入还好说最终的时候判断一下就好了，这里我们每次执行的时候都只能知道这条数据有没有成功，那怎么给前端返回最终成功多少条，失败多少条呢？
...
> 解决思路：这里就可以借用享元的思想
>
> 我们在监听器里创建一个ImportResultDTO，而后续所有的添加操作都返回本次操作的ImportResultDTO，那么我们只要最终返回监听器里的ImportResultDTO是不是就讲最终的方案实现了呢？
...
在ImportResultDTO里添加一个方法用来整合每次添加结果的数据
...
修改controller层将监听器对应的全局ImportResultDTO.getResult()返回即可
...
![image-20220604193253308](img/image-20220604193253308.png)
...
```asciiarmor
如果配置文件yml中，rule.clue.import的值为admin，则创建此对象
```
...
rule:
  ...
  clue:
    import: admin #导入时的自动分配方式--rule基于规则来分配 admin先分配给管理员再进行二次分配
```
...
```java
    ...
    public Boolean loadRule(TbClue clue) {
        //注意处理空指针的问题
        if(subjectJAVA.getDictValue().equals(clue.getSubject())){
            //如果意向学科是java--分配给zhangsan
            return distribute(clue,zhangsan);
        }else if(subjectHtml.getDictValue().equals(clue.getSubject())){
            //如果意向学科是前端--分配给zhangsan1
...
http://localhost:88/#/clue

![image-20220817164826023](img/image-20220817164826023.png)

```sql
-- 删除今天导入的数据
DELETE FROM tb_clue WHERE DATE_FORMAT(NOW(),'%Y-%m-%d') = DATE_FORMAT(create_time,'%Y-%m-%d')

-- 查询总线索数
SELECT COUNT(*) FROM tb_clue
```
...
为什么要返回url给前端呢？因为在上传完合同之后，还需要对合同进行保存，这个时候保存的合同是合同在Minio的存储的地址，在后续需要查看的时候，点击合同就能进行访问了
...
```json
{
    "msg":"操作成功",
    "code":200,
    "fileName":"/huike-crm/2022/03/18/test.pdf",
    "url":"http://127.0.0.1:9000/upload/2022/03/18/test.pdf"
}
```

```asciiarmor
注意：
返回值不再是data，而是两个自定义的属性：
fileName: 记录存储到minio中的目录+文件名
url: 具体访问地址
```
...
在application.yml中配置Minio信息：由MinioConfig进行读取，放置到IoC容器中
...
minio:
  ...
  secure: false #是否使用https，测试环境不需要
  bucketName: "huike-crm"
  configDir: "/data/excel" #目前没有用
```



 SysFileServiceImpl：上传
...
public class SysFileServiceImpl implements ISysFileService {
    ...
    public AjaxResult upload(MultipartFile file) {
        ...
        try {
            ...
            String objectName =
                // 目录名：2022/06/05/sdfsdf23423fcs231.pdf
                    new SimpleDateFormat("yyyy/MM/dd/").format(new Date()) //使用年月日作为目录
                            + UUID.randomUUID().toString().replaceAll("-", "") //UUID做文件名
            ...
            //url需要进行截取
            ajax.put("url", minioConfig.getEndpoint() + ":" + minioConfig.getPort() +
...
```asciiarmor
这部分功能出问题了！客户投诉，说Excel导入线索的时候出Bug了，现在客户那边线索导入不进去
```

> 思路：
>
> 先查看报错日志，再通过打断点的方式进行调试，看看如何解决这个问题？
